...
class TemplateClass(BaseQuery):
    ...
    def is_region_search_params_valid(self, **kwargs):
        if kwargs["intersect"] not in ("overlaps", "enclosed", "covers"):
            raise ValueError

        try:
            assert(isinstance(kwargs["region"], CircleSkyRegion))
    ...
    def query_region(self, region, intersect="overlaps", get_query_payload=False):
        try:
            self.is_region_search_params_valid(region=region, intersect=intersect)
        except TypeError:
            print("""When requesting for a region search you must specify either:
- a cone region defined by the parameters skycoord and radius
- a polygon,
- a moc,
- an url pointing to a moc""")
            return
        except ValueError:
            print("intersect parameters must have a value in ('overlaps', 'enclosed', 'covers')")
            return

        response = self.query_region_async(region, intersect, get_query_payload)
        if get_query_payload:
            return response
    ...
    def query_region_async(self, region, intersect,
            get_query_payload, cache=True):
        """
        ...
        if(isinstance(region, CircleSkyRegion)):
            request_payload = self._args_to_payload(DEC=region.center.dec.to_string(decimal=True),
                RA=region.center.ra.to_string(decimal=True),
                SR=str(region.radius.value),
                intersect=intersect,
                fmt='json')
            print(str(region.radius.value))
        else:
            # TODO manage other region types such as MOC or Polygon
            pass
